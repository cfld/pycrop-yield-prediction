...
from cyp.data import MODISExporter, DataCleaner, Engineer, EngineerConvLSTM
from cyp.models import ConvModel, RNNModel, ConvLSTMModel
...
class RunTask:
    ...
    @staticmethod
    def engineerConvLSTM(cleaned_data_path='data/img_output', yield_data_path='data/yield_data.csv',
                 county_data_path='data/county_data.csv', num_bins=32, max_bin_val=4999):
        """
        Take the preprocessed data and generate the input to the models
        ...
        county_data_path = Path(county_data_path)
        print(cleaned_data_path, yield_data_path, county_data_path)
        engineer = EngineerConvLSTM(cleaned_data_path, yield_data_path, county_data_path)
        engineer.process(num_bands=9, generate='raw_img', num_bins=num_bins, max_bin_val=max_bin_val,
                         channels_first=True)
    ...
    @staticmethod
    def train_ConvLSTM(cleaned_data_path='data/img_output', pix = 64, num_bins=32, hidden_size=128,
                  rnn_dropout=0.75, dense_features=None, savedir=Path('data/models'), times='all', pred_years=None,
                  num_runs=2, train_steps=10000, batch_size=32, starter_learning_rate=1e-3, weight_decay=0,
                  l1_weight=0, patience=10, use_gp=True, sigma=1, r_loc=0.5, r_year=1.5, sigma_e=0.32, sigma_b=0.01,
                  device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')):
